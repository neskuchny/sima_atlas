...
function list(md){ return md.split(/\r?\n/).map(s=>s.trim()).filter(s=>s.startsWith('- ')).map(s=>s.replace(/^- \[.\]\s*/, '').replace(/^- /, '').trim()); }
function nonTemplateMission(text){
  const t = (text || '').toLowerCase();
  return text && !t.includes('ключевая цель блока и его значение');
}
...
const blocks = (graph.blocks || []).map((b, i) => {
  ...
  const tasks = list(read(path.join(dir,'tasks.md'))).slice(0,3);
  const kpis = list(read(path.join(dir,'kpi.md'))).slice(0,3);
  const provides = list(read(path.join(dir,'provides.md'))).slice(0,2);
  const depends = list(read(path.join(dir,'depends_on.md'))).slice(0,2);
  return {
    id: b.id,
    title: b.title,
    status: b.status,
...
const links = (graph.blocks || []).flatMap((b) => (b.depends_on || []).map((d, idx) => {
  const src = blockMap.get(b.id);
  const dst = blockMap.get(d);
  const why = src?.depends?.[idx] || src?.depends?.[0] || 'требуется зависимость';
  const gives = dst?.provides?.[0] ? ` (получаем: ${dst.provides[0]})` : '';
  return {
    id: `${b.id}_${d}_${idx}`,
    from: b.id,
    to: d,
    label: `${b.id} -> ${d}: ${why}${gives}`,
    direction: 'to-task',
  };
}));
...
const project = {
  ...
  canvas: {
    ...
    sources: blocks.map((b) => ({
      ...
      meta: b.id,
      take: [
        nonTemplateMission(b.mission) ? b.mission : '',
        b.tasks[0] ? `что делаем: ${b.tasks[0]}` : '',
        b.kpis[0] ? `как поймем готовность: ${b.kpis[0]}` : '',
        b.provides[0] ? `что отдает: ${b.provides[0]}` : '',
      ].filter(Boolean).join(' · ') || 'нужно уточнить миссию и критерии блока',
      tags: [`#${b.status}`, '#atlas'],
    })),
    links,
  },
  map: {
    ...
    userstory: {
      ...
      nodes: blocks.map((b)=>({ id:b.id, title:b.title, kind:'блок', filled:true, body:[nonTemplateMission(b.mission)?b.mission:'', b.tasks[0] && `задача: ${b.tasks[0]}`, b.kpis[0] && `KPI: ${b.kpis[0]}`, b.depends[0] && `вход: ${b.depends[0]}`, b.provides[0] && `выход: ${b.provides[0]}`].filter(Boolean).join(' · ') || b.id, hasSubschema:false, sources:[] })),
      links: (graph.blocks || []).flatMap((b)=>(b.depends_on||[]).map(d=>({from:b.id,to:d,label:'depends_on'}))),
    }
  }
};

const payload = { data: { projects: [project] }, archByProject: { 'atlas-live': { blocks: blocks.map(({mission,tasks,kpis,provides,depends,...rest})=>({...rest, owner:'atlas'}),), links: (graph.blocks||[]).flatMap((b)=>(b.depends_on||[]).map((d)=>({from:b.id,to:d,type:'depends'}))) } } };
