...
//
// Emits frontend/atlas_bootstrap.js with one entry per discovered project.
//
...
// PR-4 (b.user-docs-generator): expose end-user docs registry to UI so
// ArchInspector can render an «Open end-user docs» button per block.
const userDocsByBlock = {};
const userDocsMetaDir = path.join(root, 'atlas', 'docs', 'end-user', '_meta');
if (fs.existsSync(userDocsMetaDir)) {
  for (const f of fs.readdirSync(userDocsMetaDir)) {
    if (!f.endsWith('.json')) continue;
    try {
      const j = JSON.parse(fs.readFileSync(path.join(userDocsMetaDir, f), 'utf8'));
      const blockId = j.block_id || path.basename(f, '.json');
      userDocsByBlock[blockId] = {
        file: `atlas/docs/end-user/${blockId}.md`,
        hash: j.hash,
        locked: !!j.locked,
...
payload.userDocsByBlock = userDocsByBlock;

// PR-7 (b.agent-orchestrator): expose ACTIVE run states so the UI can show
// a per-block "agent is running" / "stalled" / "succeeded" badge. Terminal
// runs ≥ 7d old are pruned to keep payload small.
const runStateDir = path.join(root, 'atlas', 'run_state');
const runsByBlock = {};
if (fs.existsSync(runStateDir)) {
  const cutoffMs = Date.now() - 7 * 24 * 60 * 60 * 1000;
  for (const f of fs.readdirSync(runStateDir)) {
    if (!f.endsWith('.json')) continue;
    try {
      const j = JSON.parse(fs.readFileSync(path.join(runStateDir, f), 'utf8'));
      const ts = Date.parse(j.terminated_at || j.last_event_at || j.started_at);
      if (Number.isFinite(ts) && ts < cutoffMs) continue;
      const slot = runsByBlock[j.block_id] = (runsByBlock[j.block_id] || []);
      slot.push({
        run_id: j.run_id,
        agent: j.agent,
        ...
        verifier_verdict: j.verifier_verdict || null,
        diff_proposal_id: j.diff_proposal_id || null,
        workspace_path: j.workspace_path || null,
      });
    } catch {}
  }
  for (const id of Object.keys(runsByBlock)) {
    runsByBlock[id].sort((a, b) => (b.started_at || '').localeCompare(a.started_at || ''));
  }
}
payload.runsByBlock = runsByBlock;

const outPath = path.join(root, 'frontend', 'atlas_bootstrap.js');
