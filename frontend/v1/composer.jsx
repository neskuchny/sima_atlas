    ...
    <div className={`composer ${open ? 'open' : ''}`} onClick={(e) => { if (e.target.classList.contains('composer')) onClose(); }}>
      <div className="composer-card">
        ...
        <div className="composer-bd">
          <div className="composer-input">
            ...
            <div>
              ...
              {source === 'file' && (
                <div className="drop-area">перетащите PDF/DOCX/MD сюда или <u>выберите файл</u></div>
              )}
              {source === 'meet' && (
                <div className="drop-area" style={{textAlign:'left'}}>
                  <div className="meta" style={{marginBottom:8}}>последние встречи · meetflow</div>
                  {['Планёрка 18 марта','Интервью vibe-кодер #7','Созвон с Dev команды'].map(n => (
                    <div key={n} style={{padding:'7px 9px',background:'var(--paper)',border:'0.5px solid var(--line-2)',borderRadius:5,marginBottom:5,display:'flex',alignItems:'center',gap:8,cursor:'pointer',fontSize:11}}>
                      <Icon name="meet" size={11} color="var(--ink-4)"/>
                      <span style={{flex:1,color:'var(--ink-1)'}}>{n}</span>
                      <span className="meta">добавить</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="composer-ft">
          ...
          <span className="meta">Claude · через интерфейс, без API-ключа</span>
          <button className="btn ghost sm" onClick={onClose}>Отмена</button>
          <button className="btn pri" onClick={doSynth} disabled={synthesizing}>
