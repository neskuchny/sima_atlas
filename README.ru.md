...
- ✅ **S-12** — housekeeping sweeper (R-7.88) **+ import-graph детектор мёртвого кода + вкладка Cleanup** (R-7.93): предлагает чистки stale-alive / stale-dead / stale-archived / orphan-code / `dead-code-unimported` (файл который никто не импортирует, не CLI/HTML/package entry — консервативно, 0 false-positives на нашем дереве). Вкладка «🧹 Cleanup» показывает предложения по видам с apply-командой. Остаётся nice-to-have: time-based подсказки (нет коммитов 90 дней → soft suggest)
