...
| `verify_block_acceptance(block_id)` | Run all acceptance assertions and return tri-state (pass/fail/inconclusive) per assertion — the DETERMINISTIC layer (exit codes / globs / diffs) |
| `semantic_verify {block_id, client_id?}` | **R-7.94 (Kanon «Contract as Arbiter») — the SEMANTIC layer on top.** An LLM-judge reads the whole contract (mission/kpi/acceptance/provides/depends_on) + methodology (architecture_decisions/tech_stack/rules/dont_use/always_use) + the REAL code + neighbour contracts, and judges five tri-state dimensions: **mission_fulfilled** (matches the meaning?), **conditions_met** (KPIs + acceptance intent), **methodology_followed** (honours the locks — catches «regex where an LLM was required»), **works_as_described** (functional-correctness reasoning), **connections_consistent** (provides ↔ dependents). Returns `overall` + a `todo_to_pass` list of what must change. Mock / no-key → inconclusive, never a false pass. Use after the deterministic verifier passes, to confirm the build actually means what the block says — and to get the concrete next steps to finish it. The V-1 loop runs this automatically and won't promote a block to `done` on a semantic mission/methodology FAIL. |
...
| `token_economics {days?, block?}` | **R-7.87 (S-9) — token economics roll-up.** Aggregates `atlas/llm_traces` into `{totals, top_blocks, top_ops, by_provider, daily}` with TWO cost dimensions: `cost_usd_actual` (what was charged — 0 for `claude_cli`/`ollama`/`mock`) and `cost_usd_equivalent` (Anthropic Haiku 4.5 list price — stable across providers, gives a «shadow bill» the operator can use to decide when subscription beats API). Per-block attribution via `run_state` time-window match — best-effort, returns empty if no runs recorded for that block yet. Use to find token burners before proposing optimisations. |
| `list_block_templates` / `apply_block_template {template, client_id?, block_id?, force?, dry_run?}` | **R-7.90 (S-1) — drop-in block templates.** Five production-shaped starters: `auth` · `payments` · `search` · `ingestion` · `billing`. Each is a full contract (mission + kpi + acceptance + depends_on + provides + tasks). `apply_block_template` creates the block + copies the contract so the canvas comes configured, not empty. The acceptance assertions start RED (they reference tests not yet written) — that's the verifiable definition of done; implement until green. Refuses to overwrite an existing block unless `force`. Use when the operator wants a standard subsystem rather than hand-writing a contract. |
| `agent_loop_run {dry_run?, agent?, max_iterations?, max_cost_usd?, consecutive_fail_limit?, client_id?}` | **R-7.91 (V-1) — autonomous loop daemon.** Walks the graph, picks the next RUNNABLE block (deps all `done` · non-placeholder mission · ≥1 deterministic acceptance assertion so the verifier can gate without a human), runs a fresh agent on it, verifies, and advances it ONE gated lifecycle step (idea→wip→review→done) only if the verifier passes AND no previously-green `done` block regressed. Ralph-loop shaped: progress lives on disk (graph statuses + `narrative.md`), each block gets a fresh agent. **Default `agent=print-only`** — it plans + verifies but does not spawn a real coding agent unless `agent=claude` is passed, so a first run is safe. Budget cap (`max_cost_usd`, shadow-bill), circuit breaker (`consecutive_fail_limit`), `max_iterations`, `dry_run`. Writes an Autonomous Run report to `atlas/autonomous_runs/`. **Always `dry_run:true` first** to see what it would touch. |
| `change_set_create {intent, blocks?, client_id?}` / `change_set_status {id}` / `change_set_commit {id}` / `change_set_list` | **R-7.92 (S-7) — transactional change-sets.** When a change spans several blocks (capability rename, REST→GraphQL, DB migration), group them so their acceptance is treated as ONE unit. `change_set_status` re-verifies every member and returns `all-green` / `torn`; `change_set_commit` is **refused unless every member passes** (no half-migrated state ships). Use when an edit will cascade across blocks — create the set first, drive every member green, then commit. |
