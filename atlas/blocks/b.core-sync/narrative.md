...
## 2026-09-22 — граф зависимостей: две копии, никто не сверял

**Что случилось.** В R-8.06 детектор дрейфа кода сказал: «файл блока b.db
импортирует файл блока b.acceptance-verifier-loop — объяви зависимость». Я
объявил её механически, в `depends_on.md` двух блоков. Получилось два цикла
(`b.db → b.acceptance-verifier-loop → b.db` и то же через
b.agent-orchestrator), которые развернулись в десять. Nightly прошёл 84/84.

**Почему никто не заметил.** Каждое ребро графа живёт в двух местах:
`depends_on.md` (канон, с capability) и `graph.json` (зеркало «для
скорости»). Их читают разные потребители: `cascade_verify` читает зеркало,
а `agent_loop_daemon` и детектор дрейфа кода — канон. Я обновил только канон.
С того коммита каскад и автономный цикл рассуждали о **разных графах**, и
ни одна проверка не сравнивала копии и не искала циклов.

**Правильный фикс оказался не про рёбра, а про владельца.** Детектор был
прав, что импорт есть, но вывод «объяви зависимость» был неверен.
`lifecycle_gate.mjs` — единственный писатель статусов и `transitions.log`,
вынесенное ядро `advance_block_state.mjs`, а миссия b.db прямо говорит, что он
владеет журналом переходов. Файл был зарегистрирован не в том блоке. После
переноса в b.db оба ребра стали не нужны: детектор дрейфа чист, из десяти
циклов остался один.

**Этот один — старый и другой природы.** `b.agent-orchestrator →
b.operator-profile-learner` — настоящий импорт кода. Обратно продакшн-код
профиля ничего не импортирует, он **читает данные**, которые пишет
оркестратор; единственный импорт в ту сторону — тест. `depends_on` не
различает «вызываю твой код» и «читаю то, что ты производишь», поэтому честно
объявленная пара производитель-потребитель выглядит как цикл. Записан как
обоснованное исключение с условием снятия, а не удалён.

**Что добавлено.** `validate_dependency_graph.mjs`: паритет копий, формат
зеркала (голые существующие id), циклы — всё жёстко, у каждой ошибки есть
строка `fix:`. Мутационная проверка: возврат одного ребра R-8.06 даёт ошибку
паритета и восемь циклов, exit 1. Исключение срабатывает только на точном
наборе узлов, поэтому новый цикл, проходящий через исключённую пару, всё
равно падает (группа 8 selftest'а).

**Попутно.** В `graph.json` у трёх демо-продуктовых блоков зависимости
