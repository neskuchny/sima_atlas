...
- **Semantic verification** (Kanon «Contract as Arbiter», R-7.94) — the layer *on top* of deterministic checks. An LLM-judge reads the whole contract + methodology + the real code + neighbour contracts and judges five tri-state dimensions: **does the implementation match the mission's meaning · meet the KPIs/acceptance intent · follow the methodology** (catches «wrote a regex where an LLM was required») **· will it work as described · are the block connections consistent**. Returns a `todo_to_pass` list of what must change to genuinely satisfy the contract. Mock / no-key → inconclusive, never a false pass. The autonomous loop won't promote a block to `done` on a semantic mission/methodology fail.
