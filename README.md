...
- **Semantic verification** (Kanon «Contract as Arbiter», R-7.94 + R-7.95) — the layer *on top* of deterministic checks. An LLM-judge reads the whole contract (incl. **user_story** — the top layer) + methodology + the real code + neighbour contracts and judges five tri-state dimensions: **does the implementation match the mission's meaning + the user_story · meet the KPIs/acceptance intent · follow the methodology** (catches «wrote a regex where an LLM was required») **· will it work as described · are the block connections consistent**. Returns a `todo_to_pass` list of what must change to genuinely satisfy the contract. Mock / no-key → inconclusive, never a false pass. The V-1 loop will **revisit a `done` block** if its persisted semantic verdict says mission/methodology FAILED, and inject the `todo_to_pass` into the next run's prompt — the loop genuinely «finishes blocks from what's written» (operator's exact words).
...
- ✅ **S-12** — housekeeping sweeper (R-7.88) **+ import-graph dead-code detection + Cleanup tab** (R-7.93): proposes stale-alive / stale-dead / stale-archived / orphan-code / `dead-code-unimported` (a file nothing imports, not a CLI/HTML/package entry — conservative, 0 false positives on our own tree). A «🧹 Cleanup» tab surfaces proposals grouped by kind with the apply-command for each. Remaining nice-to-have: time-based archival hints (no commits in 90d → soft suggest)
